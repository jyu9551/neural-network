...
w_output = np.random.normal(
    loc=0, scale=1, size=(output_neurons, hidden_neurons))
...
while True:
    ...
    for i in range(train_size):
        ...
        # one hot encoding
        actual_one_hot = np.zeros((10, 1))
        desired_one_hot = np.zeros((10, 1))
        actual_one_hot[np.argmax(y_d), :] = 1
        desired_one_hot[train_y[i], :] = 1
        ...
        # weight gradient
        delta_w_hidden = (-1 * x.reshape(784, 1) @
                          first_layer_bp.reshape(hidden_neurons, 1).T).T
        delta_bias_hidden = -1 * first_layer_bp
        delta_w_output = (-1 * y.reshape(hidden_neurons, 1)
                          @ last_layer_bp.reshape(1, 10)).T
...
plt.plot(range(len(train_error_list)),
         train_error_list, 'b', label='Train Data')
...
plt.plot(range(len(train_energy_list)),
         train_energy_list, 'b', label='Train Data')
plt.plot(range(len(test_energy_list)),
         test_energy_list, 'r', label='Test Data')
