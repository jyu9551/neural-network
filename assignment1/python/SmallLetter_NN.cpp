...
void initializeWeights(double weights[][numHiddenNodes], int rows, int cols) {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            // -1에서 1 사이의 난수 생성
...
////////////////
int X_train[420][imageWidth][imageHeight] = { 0, };
int X_test[20][imageWidth][imageHeight];


double w1[numInputs][numHiddenNodes];
double w2[numHiddenNodes][numOutputs] = { 0, };

double d_w1[numInputs][numHiddenNodes] = { 0, };
double d_w2[numHiddenNodes][numOutputs] = { 0, };
...
int main(void) {
    ...
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            printf("%3d ", X_train[419][i][j]);
        }
        printf("\n");
    }

    //    for (int i = 0; i < 210; i++)    printf("%d ", Y_train[i]);
    //    for (int i = 0; i < 20; i++)    printf("%d ", Y_test[i]);
